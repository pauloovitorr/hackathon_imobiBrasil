...
<body>
  ...
  <script>
    ...
    function adicionarComprador(e) {
      ...
      const novoCampo2 = document.createElement('div');

      novoCampo.classList.add('comprador');

      novoCampo.innerHTML = `
            <div>
            <label for="comprador${contador}">Selecione o comprador</label> </br>
            <select name="comprador${contador}" style='font-size:1.0rem;outline-color: #18c721 ;padding: 3px;width: 95%;' class="comprador_select">
                <option value=""></option>
                <?php
                foreach ($compradores as $comprador) {
                  echo '<option value="' . $comprador["codigo_clientes"] . '">' . $comprador['nome'] . ' CPF: ' . $comprador['cpf'] . '</option>';
...
`;
      ...
      const container = document.getElementById('form_compradores');

      pai.appendChild(novoCampo)
      pai.appendChild(novoCampo2)
    ...
    document.getElementById('add_passo').addEventListener('click', adicionarComprador);
  </script>
